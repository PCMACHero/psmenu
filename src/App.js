...
class App extends React.Component{
  ...
  render(){
    return (
      ...
      <div className={`modal ${this.state.modal?"":"animated fadeOut"}`} style={{display:""}}>
        <div className="start-btn" 
        onClick={()=>{
          this.setState({modal:false})
        }} 
        style={{color:"white",fontSize:"2em",fontFamily:"psn"}}>Start</div>
        <div className="start-btn" 
        onClick={()=>{
      ...
      <div className="App" style={{filter:`blur(${this.state.modal?"5":0}px)`}}>
        ...
        <MainRow sound={this.state.sound}/>
      </div>
